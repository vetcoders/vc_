...
/// resulting value.
pub fn expand(alloc: Allocator, io: std.Io, cmd: []const u8, path_env: ?[]const u8) !?[]u8 {
    ...
    while (it.next()) |search_path| {
        ...
        // Stat it
        const f = std.Io.Dir.openFileAbsolute(io, full_path, .{}) catch |err| switch (err) {
...
fn isExecutable(permissions: std.Io.File.Permissions) bool {
    ...
    if (@hasDecl(@TypeOf(permissions), "toMode")) {
...
test "expand: hostname" {
    ...
    const path = (try expand(testing.allocator, undefined, executable, "/bin:/usr/bin")).?;
    defer testing.allocator.free(path);
    try testing.expect(path.len > executable.len);
}

test "expand: does not exist" {
    const path = try expand(testing.allocator, undefined, "thisreallyprobablydoesntexist123", "/bin:/usr/bin");
    try testing.expect(path == null);
}

test "expand: slash" {
    const path = (try expand(testing.allocator, undefined, "foo/env", "/bin:/usr/bin")).?;
    defer testing.allocator.free(path);
