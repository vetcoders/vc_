...
fn homeUnix(buf: []u8) !?[]const u8 {
    ...
    fba.reset();
    const run = try std.process.run(fba.allocator(), std.Options.debug_io, .{
        .argv = &[_][]const u8{ "/bin/sh", "-c", "cd && pwd" },
        .stdout_limit = .limited(fba.buffer.len / 2),
        .stderr_limit = .limited(fba.buffer.len / 2),
    });
    defer fba.allocator().free(run.stdout);
    defer fba.allocator().free(run.stderr);
